...
/*
    At 15 signatures, 1/2 of the jnode network can be owned by
...
private:
    ...
    //track jnodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapJnodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageJnodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointJnode;
    std::vector<unsigned char> vchJnodeSignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointJnodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointJnode(outpointJnodeIn),
        vchJnodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointJnode);
        READWRITE(vchJnodeSignature);
    }
    ...
    COutPoint GetJnodeOutpoint() const { return outpointJnode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapJnodeVotes; // jnode outpoint - vote
...
public:
    ...
    bool HasJnodeVoted(const COutPoint& outpointJnodeIn) const;
    int CountVotes() const { return mapJnodeVotes.size(); }
...
public:
    ...
    bool HasJnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointJnodeIn);
