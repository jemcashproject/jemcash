...
struct ZerocoinTestingSetup : public TestingSetup {
    ZerocoinTestingSetup() : TestingSetup(CBaseChainParams::REGTEST, "1")
    {
        ...
        //mBlockHeightConstants["JC_V1_5_STARTING_BLOCK"] = 150;
        //mBlockHeightConstants["JC_CHECK_BUG_FIXED_AT_BLOCK"] = 140;
...
BOOST_AUTO_TEST_CASE(zerocoin_mintspend_numinputs){
    ...
    // attempt to create a zerocoin spend with more than JC_SPEND_LIMIT inputs.
    printf("Testing number of inputs for denomination %s", denominations[denominationIndexA].c_str());
    denominationsForTx.clear();

    for (int i = 0; i < (JC_SPEND_LIMIT+1)*2; i++){
    ...
    BOOST_CHECK_MESSAGE(mempool.size() == (JC_SPEND_LIMIT+1)*4, "Num input mints not added to mempool");
    ...
    BOOST_CHECK_MESSAGE(!pwalletMain->CreateZerocoinSpendModel(wtx, stringError, thirdPartyAddress, denominationsForTx), "Spend succeeded even though number of inputs > JC_SPEND_LIMIT");
