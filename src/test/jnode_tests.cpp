...
CScript scriptPubKeyJnode;


struct JnodeTestingSetup : public TestingSetup {
    JnodeTestingSetup() : TestingSetup(CBaseChainParams::REGTEST)
    {
        CPubKey newKey;
        ...
        scriptPubKeyJnode = CScript() <<  ToByteVector(newKey/*coinbaseKey.GetPubKey()*/) << OP_CHECKSIG;
        bool mtp = false;
        CBlock b;
        for (int i = 0; i < 150; i++)
        {
            std::vector<CMutableTransaction> noTxns;
            b = CreateAndProcessBlock(noTxns, scriptPubKeyJnode, mtp);
    ...
    CBlock CreateBlock(const std::vector<CMutableTransaction>& txns,
                       const CScript& scriptPubKeyJnode, bool mtp = false) {
        const CChainParams& chainparams = Params();
        CBlockTemplate *pblocktemplate = BlockAssembler(chainparams).CreateNewBlock(scriptPubKeyJnode);
    ...
    // scriptPubKeyJnode, and try to add it to the current chain.
    CBlock CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns,
                                 const CScript& scriptPubKeyJnode, bool mtp = false){

        CBlock block = CreateBlock(txns, scriptPubKeyJnode, mtp);
...
BOOST_FIXTURE_TEST_SUITE(jnode_tests, JnodeTestingSetup)

BOOST_AUTO_TEST_CASE(Test_EnforceJnodePayment)
{

    std::vector<CMutableTransaction> noTxns;
    CBlock b = CreateAndProcessBlock(noTxns, scriptPubKeyJnode, false);
    const CChainParams& chainparams = Params();
    ...
    auto const before_block = JC_JNODE_PAYMENT_BUG_FIXED_AT_BLOCK
             , after_block = JC_JNODE_PAYMENT_BUG_FIXED_AT_BLOCK + 1;
    // Emulates synced state of jnodes.
    for(size_t i =0; i < 4; ++i)
        jnodeSync.SwitchToNextAsset();


    ///////////////////////////////////////////////////////////////////////////
    // Paying to the best payee
    CJnodePayee payee1(tx.vout[1].scriptPubKey, uint256());
    // Emulates 6 votes for the payee
    for(size_t i =0; i < 5; ++i)
        payee1.AddVoteHash(uint256());

    CJnodeBlockPayees payees;
    payees.vecPayees.push_back(payee1);

    mnpayments.mapJnodeBlocks[after_block] = payees;
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Making jnodes not synchronized and checking the functionality is disabled
    jnodeSync.Reset();
    b.fChecked = false;
    b.hashMerkleRoot = BlockMerkleRoot(b, &mutated);
    while (!CheckProofOfWork(b.GetHash(), b.nBits, chainparams.GetConsensus())){
    ...
    CJnodePayee payee2(tx.vout[0].scriptPubKey, uint256());
    // Emulates 9 votes for the payee
    for(size_t i =0; i < 8; ++i)
        payee2.AddVoteHash(uint256());

    mnpayments.mapJnodeBlocks[after_block].vecPayees.insert(mnpayments.mapJnodeBlocks[after_block].vecPayees.begin(), payee2);
    ...
    mnpayments.mapJnodeBlocks[before_block] = payees;
