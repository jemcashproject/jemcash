...
UniValue dumpprivkey_jemcash(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
    {
        std::string warning =
            ...
            "Jemcash team members will never ask for this command's output and it is not needed for Jnode setup or diagnosis!\n"
            "\n"
            ...
            " Reddit: https://www.reddit.com/r/jemcash/\n"
            "\n"
            ;
...
UniValue dumpwallet_jemcash(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "dumpwallet \"filename\"\n"
