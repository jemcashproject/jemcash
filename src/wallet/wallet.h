...
enum AvailableCoinsType
{
    ...
    ONLY_1000 = 5, // find jnode outputs including locked ones (use with caution)
...
public:
    ...
    // jnode
    /// Get 1000 JEM output and keys which can be used for the Jnode
    bool GetJnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        if (ser_action.ForRead()) {
            if (!is_eof(s)) {
                ...
                if (nStoredVersion >= JC_ADVANCED_WALLETDB_MINT_VERSION)
