...
bool CWallet::AbandonTransaction(const uint256 &hashTx) {
    ...
    while (!todo.empty()) {
        ...
        if (wtx.IsZerocoinSpend()) {
            ...
            libzerocoin::CoinSpend spend(txin.nSequence >= JC_MODULUS_V2_BASE_ID ? ZCParamsV2 : ZCParams,
                                         serializedCoinSpend);
...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fJNode && pcoin->vout[i].nValue == JNODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fJNode) found = pcoin->vout[i].nValue != JNODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_1000) {
                    found = pcoin->vout[i].nValue == JNODE_COIN_REQUIRED * COIN;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fJNode && out.tx->vout[out.i].nValue == JNODE_COIN_REQUIRED * COIN) continue; //jnode input
...
bool CWallet::GetJnodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetJnodeVinAndKeys -- Could not locate any valid jnode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetJnodeVinAndKeys -- Could not locate specified jnode vin\n");
    return false;
}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        // jnode-like input should not be selected by AvailableCoins now anyway
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fJNode && wtx.vout[i].nValue == JNODE_COIN_REQUIRED * COIN) continue;
...
//                bool fFirst = true;
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 JEM.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 JEM.");
...
bool CWallet::CreateZerocoinMintModel(string &stringError, std::vector<std::pair<int,int>> denominationPairs) {
    ...
    stringError = "Zerocoin functionality has been disabled until it is fixed.";
    return false;
    ...
    BOOST_FOREACH(denominationPair, denominationPairs){
        ...
        switch(denominationValue){
            ...
            default:
                throw runtime_error(
                    "mintzerocoin <amount>(1,10,25,50,100) (\"jemcashaddress\")\n");
        }
...
bool CWallet::CreateZerocoinMintTransaction(const vector <CRecipient> &vecSend, CWalletTx &wtxNew,
	...
	// temporarily disable zerocoin
    strFailReason = "Zerocoin functionality has been disabled until it is fixed.";
    return false;
...
//                bool fAllowFree = false;					// No free TXs in JEM
    ...
    CMutableTransaction txNew;
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if(thirdPartyaddress == ""){
            	...
            	if (!address.IsValid()){
            		strFailReason = _("Invalid jemcash address");
            		return false;
            	}
...
//            LogPrintf("txNew:%s\n", txNew.ToString());
            ...
            BOOST_FOREACH(const CZerocoinEntry &minIdPubcoin, listPubCoin) {
                if (minIdPubcoin.denomination == denomination
                    ...
                    if (coinHeight > 0
                            && id < coinId
                            && coinHeight + (JC_MINT_CONFIRMATIONS-1) <= chainActive.Height()
                            && zerocoinState->GetAccumulatorValueForSpend(
                                    &chainActive,
                                    chainActive.Height()-(JC_MINT_CONFIRMATIONS-1),
                                    denomination,
                                    id,
                                    accumulatorValue,
            ...
            int serializedId = coinId + (fModulusV2 ? JC_MODULUS_V2_BASE_ID : 0);
...
string CWallet::MintAndStoreZerocoin(vector<CRecipient> vecSend, 
	...
	// temporarily disable zerocoin
    return "Zerocoin functionality has been disabled until it is fixed.";
