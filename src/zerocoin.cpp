...
bool CheckSpendJemcashTransaction(const CTransaction &tx,
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin){
        ...
        bool fModulusV2 = pubcoinId >= JC_MODULUS_V2_BASE_ID, fModulusV2InIndex = false;
        if (fModulusV2)
        ...
        if (txin.scriptSig.size() < 4)
            return state.DoS(100,
                             false,
                             REJECT_MALFORMED,
                             "CheckSpendJemcashTransaction: invalid spend transaction");
        ...
        LogPrintf("CheckSpendJemcashTransaction: tx version=%d, tx metadata hash=%s, serial=%s\n", newSpend.getVersion(), txHashForMetadata.ToString(), newSpend.getCoinSerialNumber().ToString());

        int txHeight = chainActive.Height();

        if (spendVersion == ZEROCOIN_TX_VERSION_1 && nHeight == INT_MAX) {
            int allowedV1Height = params.nSpendV15StartBlock;
            if (txHeight >= allowedV1Height + JC_V1_5_GRACEFUL_MEMPOOL_PERIOD) {
                LogPrintf("CheckSpendJemcashTransaction: cannot allow spend v1 into mempool after block %d\n",
                          allowedV1Height + JC_V1_5_GRACEFUL_MEMPOOL_PERIOD);
                return false;
            }
        }
        ...
        if (fModulusV2) {
            if ((nHeight == INT_MAX && txHeight < params.nModulusV2StartBlock) || nHeight < params.nModulusV2StartBlock)
                return state.DoS(100, false,
                                 NSEQUENCE_INCORRECT,
                                 "CheckSpendJemcashTransaction: cannon use modulus v2 at this point");
        }
        else {
            if ((nHeight == INT_MAX && txHeight >= params.nModulusV1MempoolStopBlock) ||
                ...
                return state.DoS(100, false,
                                 NSEQUENCE_INCORRECT,
                                 "CheckSpendJemcashTransaction: cannon use modulus v1 at this point");
        }
        ...
        if (!zerocoinState.GetCoinGroupInfo(targetDenominations[vinIndex], pubcoinId, coinGroup))
            return state.DoS(100, false, NO_MINT_ZEROCOIN, "CheckSpendJemcashTransaction: Error: no coins were minted with such parameters");
        ...
        do {
            if ((index->*accChanges).count(denominationAndId) > 0) {
                ...
                LogPrintf("CheckSpendJemcashTransaction: accumulator=%s\n", accumulator.getValue().ToString().substr(0,15));
        ...
        if (!passVerify && spendVersion == ZEROCOIN_TX_VERSION_1) {
            ...
            if (!passVerify) {
                ...
                BOOST_REVERSE_FOREACH(const CBigNum &pubCoin, pubCoins) {
                    accumulator += libzerocoin::PublicCoin(zcParams, pubCoin, (libzerocoin::CoinDenomination)targetDenominations[vinIndex]);
                    LogPrintf("CheckSpendJemcashTransaction: accumulatorRev=%s\n", accumulator.getValue().ToString().substr(0,15));
                    if ((passVerify = newSpend.Verify(accumulator, newMetadata)) == true)
                        break;
                }
            }
        ...
        if (!passVerify) {
            LogPrintf("CheckSpendJemcashTransaction: verification failed at block %d\n", nHeight);
            return false;
        }
    }
    ...
    if (hasZerocoinSpendInputs) {
        if (hasNonZerocoinInputs) {
            ...
            return state.DoS(100, false,
                            REJECT_MALFORMED,
                            "CheckSpendJemcashTransaction: can't mix zerocoin spend input with regular ones");
        }
            ...
            if ((nHeight == INT_MAX && txHeight < params.nMultipleSpendInputsInOneTxStartBlock) ||
                ...
                return state.DoS(100, false,
                             REJECT_MALFORMED,
                             "CheckSpendJemcashTransaction: can't have more than one input");
            }
        }
    }
...
bool CheckMintJemcashTransaction(const CTxOut &txout,
                               CValidationState &state,
                               uint256 hashTx,
                               CZerocoinTxInfo *zerocoinTxInfo) {

    LogPrintf("CheckMintJemcashTransaction txHash = %s\n", txout.GetHash().ToString());
    LogPrintf("nValue = %d\n", txout.nValue);
...
bool CheckZerocoinFoundersInputs(const CTransaction &tx, CValidationState &state, const Consensus::Params &params, int nHeight, bool fMTP) {
    ...
    if ((nHeight > params.nCheckBugFixedAtBlock) && (nHeight < params.nSubsidyHalvingInterval /*105000*/)) {
        ...
        if (nHeight < params.nJnodePaymentsStartBlock) {
            if (params.IsMain() && GetAdjustedTime() > nStartRewardTime) {
                FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("jbbQqV4axNmeAwTt3drZJm3c8EDKoMfRdc").Get());
                FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("jUXjDMj8onZr57jhdL4bdv6odDbdE1w38e").Get());
                FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("jHvNSpS4oHPoLyk9yKwGm6bR4rmrbMYx9x").Get());
                ...
                FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("TQZBxbrpdUFi4z5BM5dia1VGERwioDfb9S").Get());
                FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("TRFiBzUroPisjiVxav9g7jif7hcAqWEeom").Get());
                FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("TJwSBZibRvy9DM2BGzLnykmgrNiK8gURme").Get());
            }

            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == (int64_t)(7 * COIN)/reductionFactor) {
                    found_1 = true;
                }
                if (output.scriptPubKey == FOUNDER_2_SCRIPT && output.nValue == (int64_t)(5 * COIN)/reductionFactor) {
                    found_2 = true;
                }
                if (output.scriptPubKey == FOUNDER_3_SCRIPT && output.nValue == (int64_t)(3 * COIN)/reductionFactor) {
                    found_3 = true;
                }
            }
        } else {
            ...
            CAmount jnodePayment = GetJnodePayment(params, fMTP);
            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == (int64_t)(4 * COIN)/reductionFactor) {
                    found_1 = true;
                    continue;
                }
                ...
                if (output.scriptPubKey == FOUNDER_3_SCRIPT && output.nValue == (int64_t)(1.5 * COIN)/reductionFactor) {
                    found_3 = true;
                    continue;
                }
                if (jnodePayment == output.nValue) {
                    total_payment_tx = total_payment_tx + 1;
                }
            }

            bool validJnodePayment;

            if (nHeight > params.nJnodePaymentsBugFixedAtBlock) {
                if (!jnodeSync.IsSynced()) {
                    validJnodePayment = true;
                } else {
                    validJnodePayment = mnpayments.IsTransactionValid(tx, nHeight, fMTP);
                }
            } else {
                validJnodePayment = total_payment_tx <= 1;
            }

            if (!validJnodePayment) {
                return state.DoS(100, false, REJECT_INVALID_JNODE_PAYMENT,
                                 "CTransaction::CheckTransaction() : invalid jnode payment");
            }
        }
                              ...
                              CZerocoinTxInfo *zerocoinTxInfo)
{
    ...
    BOOST_FOREACH(const CTxOut &txout, tx.vout) {
        if (!txout.scriptPubKey.empty() && txout.scriptPubKey.IsZerocoinMint()) {
            if (!CheckMintJemcashTransaction(txout, state, hashTx, zerocoinTxInfo))
                return false;
        }
    }
    ...
    if(tx.IsZerocoinSpend()) {
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin){
            ...
            // Get the CoinDenomination value of each vin for the CheckSpendJemcashTransaction function
            uint32_t pubcoinId = txin.nSequence;
            if (pubcoinId < 1 || pubcoinId >= INT_MAX) {
                 // coin id should be positive integer
                return false;
            }
            libzerocoin::Params *zcParams = (pubcoinId >= JC_MODULUS_V2_BASE_ID) ? ZCParamsV2 : ZCParams;
        ...
        BOOST_FOREACH(const CTxOut &txout, tx.vout)
        {
            if(!isVerifyDB) {
                if (txout.nValue == totalValue * COIN) {
                    if(!CheckSpendJemcashTransaction(tx, params, denominations, state, hashTx, isVerifyDB, nHeight, isCheckWallet, fStatefulZerocoinCheck, zerocoinTxInfo)){
                        return false;
                    }
                }
                else {
...
CBigNum ZerocoinGetSpendSerialNumber(const CTransaction &tx, const CTxIn &txin) {
    ...
    try {
        CDataStream serializedCoinSpend((const char *)&*(txin.scriptSig.begin() + 4),
                                    (const char *)&*txin.scriptSig.end(),
                                    SER_NETWORK, PROTOCOL_VERSION);
        libzerocoin::CoinSpend spend(txin.nSequence >= JC_MODULUS_V2_BASE_ID ? ZCParamsV2 : ZCParams, serializedCoinSpend);
...
bool ConnectBlockZC(CValidationState &state, const CChainParams &chainParams, CBlockIndex *pindexNew, const CBlock *pblock, bool fJustCheck) {
    ...
    if (pblock && pblock->zerocoinTxInfo) {
        if (pblock->zerocoinTxInfo->fHasSpendV1) {
            ...
            if (pindexNew->nHeight >= allowV1Height + JC_V1_5_GRACEFUL_PERIOD) {
...
int CZerocoinState::AddMint(CBlockIndex *index, int denomination, const CBigNum &pubCoin, CBigNum &previousAccValue) {
    ...
    int coinsPerId = IsZerocoinTxV2((libzerocoin::CoinDenomination)denomination,
                        Params().GetConsensus(), mintId) ? JC_SPEND_V2_COINSPERID : JC_SPEND_V1_COINSPERID;
