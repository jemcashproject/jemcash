...
#if defined(NDEBUG)
# error "Jemcash cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Jemcash Signed Message:\n";
...
bool AcceptToMemoryPoolWorker(
        ...
        bool markJemcashSpendTransactionSerial) {
    ...
    if (tx.IsZerocoinSpend() && markJemcashSpendTransactionSerial)
...
bool AcceptToMemoryPool(
    ...
    bool res = AcceptToMemoryPoolWorker(
        ...
        markJemcashSpendTransactionSerial);
    if (res) {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams, int nTime) {
    ...
    // Subsidy is cut in half every 416,666 blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;
...
CAmount GetJnodePayment(const Consensus::Params &params, bool fMTP) {

	CBlockIndex *pindex = chainActive.Tip();
	CAmount ret = 0;
	CAmount coin = fMTP ? COIN/params.nMTPRewardReduction : COIN;
	
	CAmount blockValue = GetBlockSubsidy(pindex->nHeight+1, Params().GetConsensus(), pindex->nTime);
	
	if (pindex->nHeight+1 < params.nSubsidyHalvingInterval) {
    	ret = 37.5 * coin;
	}else {
		ret = ( blockValue * 0.3 );
	}
...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (txIds.count(txHash) > 0 && (fTestNet || pindex->nHeight >= HF_JNODE_HEIGHT))
    ...
    // JNODE : MODIFIED TO CHECK JNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward, block.IsMTP())) {
        ...
        return state.DoS(0, error("ConnectBlock(): couldn't find jnode or superblock payments"),
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams) {
    ...
    jnodeSync.UpdatedBlockTip(chainActive.Tip());
        ...
        BOOST_FOREACH(const CTransaction &tx, block.vtx) {
            ...
            if (!tx.IsCoinBase()) {
                AcceptToMemoryPool(
                    ...
                    false /* markJemcashSpendTransactionSerial */
                );
            }
...
bool CheckBlock(const CBlock &block, CValidationState &state,
    ...
    try {
        ...
        if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
            ...
            BOOST_FOREACH(const CTransaction& tx, block.vtx) {
                ...
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    ...
                    if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                        ...
                        LOCK(cs_main);
                        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                        return state.DoS(0, error("CheckBlock(JEM): transaction %s conflicts with transaction lock %s",
            ...
            LogPrintf("CheckBlock(JEM): spork is off, skipping transaction locking checks\n");
        }
...
bool ProcessNewBlock(CValidationState &state, const CChainParams &chainparams, CNode *pfrom, const CBlock *pblock,
    ...
    jnodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    switch (inv.type) {
        ...
        */
        ...
        case MSG_JNODE_PAYMENT_VOTE:
            return mnpayments.mapJnodePaymentVotes.count(inv.hash);

        case MSG_JNODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapJnodeBlocks.find(mi->second->nHeight) != mnpayments.mapJnodeBlocks.end();
        }

        case MSG_JNODE_ANNOUNCE:
            return mnodeman.mapSeenJnodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

        case MSG_JNODE_PING:
            return mnodeman.mapSeenJnodePing.count(inv.hash);

        case MSG_DSTX:
            return mapDarksendBroadcastTxes.count(inv.hash);

        case MSG_JNODE_VERIFY:
            return mnodeman.mapSeenJnodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK ||
                ...
                if (!pushed && inv.type == MSG_JNODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapJnodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::JNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_JNODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapJnodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapJnodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CJnodePayee& payee, mnpayments.mapJnodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapJnodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::JNODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_JNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenJnodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenJnodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_JNODE_PING) {
                    if(mnodeman.mapSeenJnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenJnodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_JNODE_VERIFY) {
                    if(mnodeman.mapSeenJnodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenJnodeVerification[inv.hash];
...
bool static ProcessMessage(CNode *pfrom, string strCommand,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion = (nHeight + 1 < JC_MODULUS_V2_START_BLOCK) ? MIN_PEER_PROTO_VERSION : MIN_PEER_PROTO_VERSION_AFTER_JNODE_PAYMENT_HF;
...
//        vRecv >> tx;
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CJnode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find jnode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- Jnode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeyJnode)) {
...
//        LogPrintf("Main.cpp ProcessMessage() strCommand=%s\n", strCommand);
        ...
        if (found) {
            ...
            jnodeSync.ProcessMessage(pfrom, strCommand, vRecv);
        } else {
            // Ignore unknown commands for extensibility
