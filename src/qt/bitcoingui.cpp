...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setStatusTip(tr(exodusEnabled ? "Send Exodus and Jemcash transactions" : "Send coins to a Jemcash address"));
    ...
    jnodeAction = new QAction(platformStyle->SingleColorIcon(":/icons/jnodes"), tr("&Jnodes"), this);
    jnodeAction->setStatusTip(tr("Browse jnodes"));
    jnodeAction->setToolTip(jnodeAction->statusTip());
    jnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
    jnodeAction->setShortcut(QKeySequence(Qt::CTRL + key++));
#else
    jnodeAction->setShortcut(QKeySequence(Qt::ALT +  key++));
#endif
    tabGroup->addAction(jnodeAction);
#endif
...
#ifdef ENABLE_WALLET
    connect(jnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(jnodeAction, SIGNAL(triggered()), this, SLOT(gotoJnodePage()));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    jnodeAction->setEnabled(enabled);
...
void BitcoinGUI::gotoJnodePage()
{
    QSettings settings;
    jnodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoJnodePage();
}
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    if(!jnodeSync.IsBlockchainSynced())
    {
...
void BitcoinGUI::setAdditionalDataSyncProgress(int count, double nSyncProgress)
{
    ...
    if(jnodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(jnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Jemcash"); // default title
