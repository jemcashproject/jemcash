...
void SendMPDialog::updateFrom()
{
    ...
    if (currentSetFromAddress.empty()) {
        ...
        if (CheckFee(currentSetFromAddress, 16)) {
            ...
            ui->feeWarningLabel->setText("WARNING: The sending address is low on JEM for transaction fees. Please topup the JEM balance for the sending address to send Exodus transactions.");
