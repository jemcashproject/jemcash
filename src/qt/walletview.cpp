    ...
    sendCoinsTabs(0)
{
    ...
    jnodeListPage = new JnodeList(platformStyle);
...
{
    // Create Jemcash transactions list
    jemcashTransactionList = new TransactionView(platformStyle);

    connect(jemcashTransactionList, SIGNAL(doubleClicked(QModelIndex)), jemcashTransactionList, SLOT(showDetails()));
    connect(jemcashTransactionList, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));

    // Create export panel for Jemcash transactions
    ...
    connect(exportButton, SIGNAL(clicked()), jemcashTransactionList, SLOT(exportClicked()));
    ...
    auto jemcashLayout = new QVBoxLayout();
    jemcashLayout->addWidget(jemcashTransactionList);
    jemcashLayout->addLayout(exportLayout);

    jemcashTransactionsView = new QWidget();
    jemcashTransactionsView->setLayout(jemcashLayout);
    ...
    if (isExodusEnabled()) {
        ...
        transactionTabs->addTab(jemcashTransactionsView, tr("Jemcash"));
    ...
    if (transactionTabs) {
        ...
        pageLayout->addWidget(jemcashTransactionsView);
    }
...
{
    sendJemcashView = new SendCoinsDialog(platformStyle);

    connect(sendJemcashView, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
    ...
    if (isExodusEnabled()) {
        ...
        sendCoinsTabs->addTab(sendJemcashView, tr("Jemcash"));
    ...
    if (sendCoinsTabs) {
        ...
        pageLayout->addWidget(sendJemcashView);
    }
...
void WalletView::setClientModel(ClientModel *clientModel)
{
    ...
    sendJemcashView->setClientModel(clientModel);
    jnodeListPage->setClientModel(clientModel);
...
void WalletView::setWalletModel(WalletModel *walletModel)
{
    ...
    jemcashTransactionList->setModel(walletModel);
    overviewPage->setWalletModel(walletModel);
    receiveCoinsPage->setModel(walletModel);
    zerocoinPage->setModel(walletModel->getAddressTableModel());
    usedReceivingAddressesPage->setModel(walletModel->getAddressTableModel());
    usedSendingAddressesPage->setModel(walletModel->getAddressTableModel());
    jnodeListPage->setWalletModel(walletModel);
    sendJemcashView->setModel(walletModel);
...
void WalletView::focusBitcoinHistoryTab(const QModelIndex &idx)
{
    ...
    jemcashTransactionList->focusTransaction(idx);
}

void WalletView::gotoJnodePage()
{
    setCurrentWidget(jnodeListPage);
}
...
void WalletView::gotoSendCoinsPage(QString addr)
{
    ...
    if (!addr.isEmpty()){
        sendJemcashView->setAddress(addr);
    }
}
...
bool WalletView::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    ...
    return sendJemcashView->handlePaymentRequest(recipient);
}
