...
// step 2) Top COutPointLock::SIGNATURES_TOTAL jnodes per each spent outpoint push "txvote" message
...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until jnode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Jnodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetJnodeRank(activeJnode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Jnode %s\n", activeJnode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Jnode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasJnodeVoted(itOutpointLock->first, activeJnode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeJnode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  jnode=%s new\n",
                    txHash.ToString(), vote.GetJnodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  jnode=%s seen\n",
                    txHash.ToString(), vote.GetJnodeOutpoint().ToStringShort());
        }
        ...
        int nJnodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapJnodeOrphanVotes.count(vote.GetJnodeOutpoint())) {
            mapJnodeOrphanVotes[vote.GetJnodeOutpoint()] = nJnodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapJnodeOrphanVotes[vote.GetJnodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageJnodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- jnode is spamming orphan Transaction Lock Votes: txid=%s  jnode=%s\n",
                        txHash.ToString(), vote.GetJnodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasJnodeVoted(vote.GetOutpoint(), vote.GetJnodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same jnode.
                    // TODO: apply pose ban score to this jnode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- jnode sent conflicting votes! %s\n", vote.GetJnodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other jnodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageJnodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapJnodeOrphanVotes is empty
    if(mapJnodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapJnodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapJnodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapJnodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  jnode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetJnodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  jnode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetJnodeOutpoint().ToStringShort());
    ...
    // remove expired jnode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itJnodeOrphan = mapJnodeOrphanVotes.begin();
    while(itJnodeOrphan != mapJnodeOrphanVotes.end()) {
        if(itJnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan jnode vote: jnode=%s\n",
                    itJnodeOrphan->first.ToStringShort());
            mapJnodeOrphanVotes.erase(itJnodeOrphan++);
        } else {
            ++itJnodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointJnode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown jnode %s\n", outpointJnode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointJnode));
        return false;
    }
    ...
    int n = mnodeman.GetJnodeRank(CTxIn(outpointJnode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated jnode %s\n", outpointJnode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Jnode %s, rank=%d\n", outpointJnode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Jnode %s is not in the top %d (%d), vote hash=%s\n",
                outpointJnode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    jnode_info_t infoMn = mnodeman.GetJnodeInfo(CTxIn(outpointJnode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Jnode: jnode=%s\n", outpointJnode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyJnode, vchJnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchJnodeSignature, activeJnode.keyJnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeJnode.pubKeyJnode, vchJnodeSignature, strMessage, strError)) {
...
{
    if(mapJnodeVotes.count(vote.GetJnodeOutpoint()))
        return false;
    mapJnodeVotes.insert(std::make_pair(vote.GetJnodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapJnodeVotes.begin();
    while(itVote != mapJnodeVotes.end()) {
...
bool COutPointLock::HasJnodeVoted(const COutPoint& outpointJnodeIn) const
{
    return mapJnodeVotes.count(outpointJnodeIn);
}
...
bool CTxLockCandidate::HasJnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointJnodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasJnodeVoted(outpointJnodeIn);
}
