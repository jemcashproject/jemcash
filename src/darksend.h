...
// A helper object for signing messages from Jnodes
...
public:
     ...
     *     1) we have an active Jnode,
     *     2) we have a valid Jnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Jnode address
    bool CheckSignature(const CPubKey& pubKeyJnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, jnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 JEM - checking if valid jnode)
...
private:
    ...
    // Keep track of the used Jnodes
    std::vector<CTxIn> vecJnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Jnode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Jnode to start the process
    ...
    /// Get Jnode updates about the progress of mixing
