#ifndef ZEROCOIN_PARAMS_H
#define ZEROCOIN_PARAMS_H

#define ZEROCOIN_MODULUS   ""
#define ZEROCOIN_MODULUS_V2 ""

/** Dust Soft Limit, allowed with additional fee per output */
//static const int64_t DUST_SOFT_LIMIT = 100000; // 0.001 JEM
/** Dust Hard Limit, ignored as wallet inputs (mininput default) */
static const int64_t DUST_HARD_LIMIT = 1000;   // 0.00001 JEM mininput

// There were bugs before this block, don't do some checks on early blocks
#define JC_CHECK_BUG_FIXED_AT_BLOCK         1

// Before this block we allowed not paying to the jnodes.
#define JC_JNODE_PAYMENT_BUG_FIXED_AT_BLOCK         8640

// Do strict check on duplicate minted public coin value after this block
#define JC_CHECK_DUPLICATE_MINT_AT_BLOCK    1

// The mint id number to change to zerocoin v2
#define JC_V2_SWITCH_ID_1 200
#define JC_V2_SWITCH_ID_10 30
#define JC_V2_SWITCH_ID_25 15
#define JC_V2_SWITCH_ID_50 15
#define JC_V2_SWITCH_ID_100 100

// Same for testnet
#define JC_V2_TESTNET_SWITCH_ID_1 18
#define JC_V2_TESTNET_SWITCH_ID_10 7
#define JC_V2_TESTNET_SWITCH_ID_25 5
#define JC_V2_TESTNET_SWITCH_ID_50 4
#define JC_V2_TESTNET_SWITCH_ID_100 10

#define JC_V1_5_STARTING_BLOCK          1
#define JC_V1_5_TESTNET_STARTING_BLOCK  1

#define JC_V1_5_GRACEFUL_MEMPOOL_PERIOD	500
#define JC_V1_5_GRACEFUL_PERIOD			1500

#define JC_MODULUS_V2_START_BLOCK		15000
#define JC_MODULUS_V1_MEMPOOL_STOP_BLOCK        15500
#define JC_MODULUS_V1_STOP_BLOCK		15800

#define JC_MODULUS_V2_TESTNET_START_BLOCK       100
#define JC_MODULUS_V1_TESTNET_MEMPOOL_STOP_BLOCK 150
#define JC_MODULUS_V1_TESTNET_STOP_BLOCK        180

#define JC_MODULUS_V2_BASE_ID			1000

#define JC_MULTIPLE_SPEND_INPUT_STARTING_BLOCK  12000

// Number of coins per id in spend v1/v1.5
#define JC_SPEND_V1_COINSPERID			10
// Number of coins per id in spend v2.0
#define JC_SPEND_V2_COINSPERID			10000

// Version of index that introduced storing accumulators and coin serials
#define JC_ADVANCED_INDEX_VERSION           130500
// Version of wallet.db entry that introduced storing extra information for mints
#define JC_ADVANCED_WALLETDB_MINT_VERSION	130504

// number of mint confirmations needed to spend coin
#define JC_MINT_CONFIRMATIONS               6

// Genesis block timestamp
#define JC_GENESIS_BLOCK_TIME               1554465600

#define SWITCH_TO_MTP_BLOCK_HEADER 1554466800
#define SWITCH_TO_MTP_5MIN_BLOCK            1

// Number of zerocoin spends allowed per block and per transaction
#define JC_SPEND_LIMIT         5

// Number of zerocoin mints allowed per transaction
#define JC_MINT_LIMIT         100
#endif
