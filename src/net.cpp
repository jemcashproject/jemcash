...
bool AcceptToMemoryPool(
        ...
        bool markJemcashSpendTransactionSerial = true);
...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fConnectToJnode) {
    if (pszDest == NULL) {
        // we clean jnode connections in CJnodeMan::ProcessJnodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveJnode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToJnode)
            return NULL;
        LOCK(cs_vNodes);
        // Look for an existing connection
        CNode *pnode = FindNode((CService) addrConnect);
        if (pnode) {
            // we have existing connection to this node but it was not a connection to jnode,
            // change flag and add reference so that we can correctly clear it later
            if (fConnectToJnode && !pnode->fJnode) {
                pnode->fJnode = true;
            }
            pnode->AddRef();
            return pnode;
...
void ThreadDandelionShuffle() {
    ...
    while (!CNode::interruptNet) {
        if (GetTimeMicros() > nNextDandelionShuffle) {
            ...
            // results to not being able to close jemcash.
