...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if ((nHeight > 0) && (nHeight < params.nSubsidyHalvingInterval)) {
        ...
        if (nHeight < params.nJnodePaymentsStartBlock) {
            ...
            coinbaseTx.vout.push_back(CTxOut(7 * coin, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(5 * coin, CScript(FOUNDER_2_SCRIPT.begin(), FOUNDER_2_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(3 * coin, CScript(FOUNDER_3_SCRIPT.begin(), FOUNDER_3_SCRIPT.end())));
        } else if (nHeight >= Params().GetConsensus().nJnodePaymentsStartBlock) {
            // Take some reward away from us
            coinbaseTx.vout[0].nValue = -7.5 * coin;
            ...
            // And give it to the founders
            coinbaseTx.vout.push_back(CTxOut(4 * coin, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(2 * coin, CScript(FOUNDER_2_SCRIPT.begin(), FOUNDER_2_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(1.5 * coin, CScript(FOUNDER_3_SCRIPT.begin(), FOUNDER_3_SCRIPT.end())));
        }
    }
...
//            }
            ...
            if (tx.IsZerocoinSpend()) {
                ...
                LogPrintf("COUNT_SPEND_JC_TX =%s\n", COUNT_SPEND_JC_TX);
                LogPrintf("MAX_SPEND_JC_TX_PER_BLOCK =%s\n", MAX_SPEND_JC_TX_PER_BLOCK);
                if ((COUNT_SPEND_JC_TX + tx.vin.size()) > MAX_SPEND_JC_TX_PER_BLOCK) {
                    continue;
                }
        ...
        // Update coinbase transaction with additional info about jnode and governance payments,
        // get some info back to pass to getblocktemplate
        if (nHeight >= chainparams.GetConsensus().nJnodePaymentsStartBlock) {
            const Consensus::Params &params = chainparams.GetConsensus();
            CAmount jnodePayment = GetJnodePayment(chainparams.GetConsensus(), nHeight > 0 && nBlockTime >= params.nMTPSwitchTime);
            coinbaseTx.vout[0].nValue -= jnodePayment;
            FillBlockPayments(coinbaseTx, nHeight, jnodePayment, pblock->txoutJnode, pblock->voutSuperblock);
        }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL, false))
        return error("JemcashMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static JemcashMiner(const CChainParams &chainparams) {
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("JemcashMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                // Also try to wait for jnode winners unless we're on regtest chain
                do {
                    bool fvNodesEmpty;
                    bool fHasJnodesWinnerForNextBlock;
                    const Consensus::Params &params = chainparams.GetConsensus();
                    {
                        LOCK(cs_vNodes);
                    ...
                    }
                    {
                        LOCK2(cs_main, mempool.cs);
                        int nCount = 0;
                        fHasJnodesWinnerForNextBlock = 
                                params.IsRegtest() ||
                                chainActive.Height() < params.nJnodePaymentsStartBlock ||
                                mnodeman.GetNextJnodeInQueueForPayment(chainActive.Height(), true, nCount);
                    }
                    if (!fvNodesEmpty && fHasJnodesWinnerForNextBlock && !IsInitialBlockDownload()) {
                        break;
                    }
                    MilliSleep(1000);
                } while (true);
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in JemcashMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running JemcashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        CheckWork(pblock, *pwallet, reservekey);
        ...
        LogPrintf("JemcashMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("JemcashMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JemcashMiner, boost::cref(chainparams)));
}
