...
UniValue exodus_getactivedexsells(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"jemcashdesired\" : \"n.nnnnnnnn\",    (string) the number of jemcash desired in exchange\n"
    ...
    for (OfferMap::iterator it = my_offers.begin(); it != my_offers.end(); ++it) {
        ...
        int64_t sellBitcoinDesired = selloffer.getJEMDesiredOriginal(); //badly named - "Original" implies off the wire, but is amended amount
        ...
        responseObj.push_back(Pair("jemcashdesired", FormatDivisibleMP(bitcoinDesired)));
        ...
        for (AcceptMap::const_iterator ait = my_accepts.begin(); ait != my_accepts.end(); ++ait) {
            ...
            if (accept.getHash() == selloffer.getHash()) {
                ...
                int64_t amountToPayInBTC = calculateDesiredBTC(accept.getOfferAmountOriginal(), accept.getJEMDesiredOriginal(), amountAccepted);
...
UniValue exodus_getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"jemcashcoreversion\" : \"x.x.x\",        (string) Bitcoin Core version\n"
    ...
    infoResponse.push_back(Pair("jemcashcoreversion", JemcashCoreVersion()));
