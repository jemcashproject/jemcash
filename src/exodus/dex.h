...
private:
    ...
    uint32_t property;
    //! amount desired, in JEM
    int64_t JEM_desired_original;
    int64_t min_fee;
...
public:
    ...
    int64_t getJEMDesiredOriginal() const { return JEM_desired_original; }

    CMPOffer()
      : offerBlock(0), offer_amount_original(0), property(0), JEM_desired_original(0), min_fee(0),
    ...
    CMPOffer(int block, int64_t amountOffered, uint32_t propertyId, int64_t amountDesired,
      ...
      : offerBlock(block), offer_amount_original(amountOffered), property(propertyId),
        JEM_desired_original(amountDesired), min_fee(minAcceptFee), blocktimelimit(paymentWindow),
        txid(tx), subaction(0)
    {
    ...
    CMPOffer(const CMPTransaction& tx)
      : offerBlock(tx.block), offer_amount_original(tx.nValue), property(tx.property),
        JEM_desired_original(tx.amount_desired), min_fee(tx.min_fee),
...
private:
    ...
    int64_t JEM_desired_original;      // copied from the Offer during Accept's creation
...
public:
    ...
    CMPAccept(int64_t amountAccepted, int blockIn, uint8_t paymentWindow, uint32_t propertyId,
      ...
      : accept_amount_remaining(amountAccepted), blocktimelimit(paymentWindow),
        ...
        JEM_desired_original(amountDesired), offer_txid(txid), block(blockIn)
    {
