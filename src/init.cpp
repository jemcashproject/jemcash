...
void Shutdown() {
    ...
    CFlatDB<CJnodeMan> flatdb1("jncache.dat", "magicJnodeCache");
    flatdb1.Dump(mnodeman);
    CFlatDB<CJnodePayments> flatdb2("jnpayments.dat", "magicJnodePaymentsCache");
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE = "<https://github.com/jemcashproject/jemcash>";
    const std::string URL_WEBSITE = "<https://jemcash.info/>";
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
        ...
        do {
            try {
	            ...
	            if (!fReindex) {
                    ...
                    if (pblocktree->GetBlockIndexVersion() < JC_ADVANCED_INDEX_VERSION) {
    ...
    fJNode = GetBoolArg("-jnode", false);

    LogPrintf("fJNode = %s\n", fJNode);
    LogPrintf("jnodeConfig.getCount(): %s\n", jnodeConfig.getCount());

    if ((fJNode || jnodeConfig.getCount() > 0) && !fTxIndex) {
        return InitError("Enabling Jnode support requires turning on transaction indexing."
    ...
    if (fJNode) {
        ...
        if (!GetArg("-jnodeaddr", "").empty()) {
            // Hot Jnode (either local or remote) should get its address in
            // CActiveJnode::ManageState() automatically and no longer relies on Jnodeaddr.
            return InitError(_("jnodeaddr option is deprecated. Please use jnode.conf to manage your remote jnodes."));
        }

        std::string strJnodePrivKey = GetArg("-jnodeprivkey", "");
        if (!strJnodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strJnodePrivKey, activeJnode.keyJnode,
                                                  activeJnode.pubKeyJnode))
                return InitError(_("Invalid jnodeprivkey. Please see documentation."));

            LogPrintf("  pubKeyJnode: %s\n", CBitcoinAddress(activeJnode.pubKeyJnode.GetID()).ToString());
        } else {
            return InitError(
                    _("You must specify a jnodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using Jnode config file %s\n", GetJnodeConfigFile().string());

    if (GetBoolArg("-jnconflock", true) && pwalletMain && (jnodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Jnodes:\n");
        uint256 mnTxHash;
        int outputIndex;
...
//    nInstantSendDepth = std::min(std::max(nInstantSendDepth, 0), 60);
    ...
    // lite mode disables all Jnode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fJNode && fLiteMode) {
        return InitError("You can not start a jnode in litemode");
    }
...
//    LogPrintf("PrivateSend amount %d\n", nPrivateSendAmount);
    ...
    if (GetBoolArg("-persistentjnodestate", true)) {
        uiInterface.InitMessage(_("Loading jnode cache..."));
        CFlatDB<CJnodeMan> flatdb1("jncache.dat", "magicJnodeCache");
        if (!flatdb1.Load(mnodeman)) {
            return InitError("Failed to load jnode cache from jncache.dat");
        }

        if (mnodeman.size()) {
            uiInterface.InitMessage(_("Loading Jnode payment cache..."));
            CFlatDB<CJnodePayments> flatdb2("jnpayments.dat", "magicJnodePaymentsCache");
            if (!flatdb2.Load(mnpayments)) {
                return InitError("Failed to load jnode payments cache from jnpayments.dat");
            }
        } else {
            uiInterface.InitMessage(_("Jnode cache is empty, skipping payments cache..."));
        }
