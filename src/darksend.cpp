...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fJNode) {
            LogPrintf("DSACCEPT -- not a Jnode!\n");
        ...
        CJnode *pmn = mnodeman.Find(activeJnode.vin);
        if (pmn == NULL) {
        ...
        CJnode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyJnode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToJnode) return;
            if ((CNetAddr) pSubmittedToJnode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Jnode: pSubmittedToJnode=%s, addr=%s\n", pSubmittedToJnode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on jnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Jnode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Jnode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from jnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToJnode && pSubmittedToJnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (fJNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Jnode!\n");
            return;
        }

        if (!pSubmittedToJnode) return;
        if ((CNetAddr) pSubmittedToJnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Jnode: pSubmittedToJnode %s addr %s\n", pSubmittedToJnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fJNode) {
            LogPrintf("DSSIGNFINALTX -- not a Jnode!\n");
            return;
        }
        ...
        if (fJNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Jnode!\n");
            return;
        }

        if (!pSubmittedToJnode) return;
        if ((CNetAddr) pSubmittedToJnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Jnode: pSubmittedToJnode %s addr %s\n", pSubmittedToJnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fJNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Jnode!\n");
            return;
        }

        if (!pSubmittedToJnode) return;
        if ((CNetAddr) pSubmittedToJnode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Jnode: pSubmittedToJnode=%s  addr=%s\n", pSubmittedToJnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !jnodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to jnode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to jnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to jnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a Jnode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fJNode) return; // check and relay final tx only on jnode
    ...
    // create and sign jnode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeJnode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Jnodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Jnode
...
void CDarksendPool::CheckTimeout() {
    ...
    if (!fEnablePrivateSend && !fJNode) return;
    ...
    int nLagTime = fJNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeJnode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Jnode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fJNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Jnode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Jnode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Jnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Jnode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Jnode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Jnode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the Jnode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the jnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    if (!fEnablePrivateSend || fJNode || !pCurrentBlockIndex) return false;
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Jnodes detected\n");
        strAutoDenomResult = _("No Jnodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Jnode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecJnodesUsed: size: %d, threshold: %d\n", (int) vecJnodesUsed.size(), nThreshold_high);

    if ((int) vecJnodesUsed.size() > nThreshold_high) {
        vecJnodesUsed.erase(vecJnodesUsed.begin(), vecJnodesUsed.begin() + vecJnodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecJnodesUsed: new size: %d, threshold: %d\n", (int) vecJnodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CJnode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq jnode is not in jnode list, jnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecJnodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to jnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Jnode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fJnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToJnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Jnode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CJnode *pmn = mnodeman.FindRandomNotInVec(vecJnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random jnode!\n");
            strAutoDenomResult = _("Can't find random Jnode.");
            return false;
        }
        vecJnodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this jnode!"
                              " jnode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Jnode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fJnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible Jnode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fJNode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeJnode.vin, GetTime(), false);
...
bool CDarksendPool::AddUserToExistingSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fJNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the Jnode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a Jnode.");
        case ERR_QUEUE_FULL:
            return _("Jnode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == JNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeJnode.pubKeyJnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyJnode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyJnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Jnode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyJnode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToJnode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fJNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Jnode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && jnodeSync.IsJnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        jnodeSync.ProcessTick();

        if (jnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all jnodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % JNODE_MIN_MNP_SECONDS == 15)
                activeJnode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessJnodeConnections();
                mnodeman.CheckAndRemove();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if (fJNode && (nTick % (60 * 5) == 0)) {
