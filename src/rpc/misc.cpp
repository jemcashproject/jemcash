...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"jemcashaddress\"\n"
            "\nReturn information about the given jemcash address.\n"
            "\nArguments:\n"
            "1. \"jemcashaddress\"     (string, required) The jemcash address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"jemcashaddress\", (string) The jemcash address validated\n"
...
UniValue jnsync(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "jnsync [status|next|reset]\n"
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", jnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", jnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", jnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", jnodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsJnodeListSynced", jnodeSync.IsJnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", jnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", jnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", jnodeSync.IsFailed()));
        return objStatus;
    }
    ...
    if(strMode == "next")
    {
        ...
        return "sync updated to " + jnodeSync.GetAssetName();
    }
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"jemcashaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"jemcashaddress\"  (string, required) The jemcash address to use for the signature.\n"
    ...
    for(idx_rec const & idr : addressIndex) {
        ...
        for(CTxIn const & in : tx.vin) {
            ...
            try {
                ...
                libzerocoin::CoinSpend spend(in.nSequence >= JC_MODULUS_V2_BASE_ID ? ZCParamsV2 : ZCParams, serializedCoinSpend);
