# How to compile and execute Jemcash code on Windows 10

There are two possibilities of compiling Jemcash code on Windows 10.
 ...
 * [Xming](http://www.straightrunning.com/XmingNotes/) latest version, if you are planning to compile using [unix compiling instruction](https://github.com/jemcashproject/jemcash/blob/master/doc/build-unix.md)
...
4. Make shared folder between two locations
    ...
    * From Explorer create a directory named:
            ...
            C:\Users\<username>\jemcash
    * From Ubuntu bash window create a link to it:
  
            ln -s /mnt/c/Users/<username>/jemcash jemcash   
...
5. Clone git repository into newly created directory and go to the right branch
            ...
            git clone https://github.com/jemcashproject/jemcash.git
            cd jemcash
...
3. Go to depends folder and build dependencies (you may wish to build only 32-bit or 64-bit version)
        ...
        cd ~/jemcash
    It takes a while. You need to do it only once unless you delete jemcash-depends directory
...
6. Build 32-bit debug build (from the jemcash root directory)
    
        ./configure --prefix=$HOME/jemcash-depends/i686-w64-mingw32
        make -j`nproc`

7. Build 64-bit debug build (from the jemcash root directory)
     
        ./configure --prefix=$HOME/jemcash-depends/x86_64-w64-mingw32
        make -j`nproc`

8. After check the directory to run GUI with __jemcash-qt.exe__
    
        C:\Users\<username>\jemcash\src\qt
----
## Second method - Ubuntu + Xming
1. Use existing paper [build-unix](https://github.com/jemcashproject/jemcash/blob/master/doc/build-unix.md) and
...
4. From Ubuntu bash window start jemcash-qt:

        cd /mnt/c/Users/<username>/jemcash/src/qt
        ./jemcash-qt 
